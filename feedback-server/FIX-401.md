...
4. In the KnowledgeCache app, open **`KnowledgeCache/Feedback/FeedbackConfig.swift`** and set the secret:
   ...
   static var protectionBypassSecret: String {
       "" // paste the exact bypass secret value from Vercel
   }
   ```
...
**If using Option B (bypass secret):**
```bash
curl -X POST "https://feedback-server-psi.vercel.app/api/feedback" \
...
**If using Option A (public production):**
```bash
curl -X POST "https://feedback-server-psi.vercel.app/api/feedback" \
  -H "Content-Type: application/json" \
  -d '{"id":"test","message":"test","email":null,"type":"bug","app_version":"1.0","os_version":"14.0","timestamp":"2025-02-10T22:00:00.000Z"}'
...
- **401** -> protection still blocking; add bypass secret (Option B) or make production public (Option A).
