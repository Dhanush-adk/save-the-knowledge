...
node --check feedback-server/api/investor-kpis.js
node --check feedback-server/api/investor-export.js
...
node -e "const {computeInvestorKpis, generateInvestorSnapshots, generateRetentionCohorts, computeOpsSignals}=require('./feedback-server/lib/kpis'); const rows=[{event:'session_started',install_id:'a',activated:true,timestamp:'2026-02-12T00:00:00Z'},{event:'url_saved',install_id:'a',timestamp:'2026-02-12T00:10:00Z'},{event:'query_answered',install_id:'a',query_success:true,query_latency_ms:210,timestamp:'2026-02-12T00:11:00Z'},{event:'query_answered',install_id:'a',timestamp:'2026-02-13T00:00:00Z'}]; const out=computeInvestorKpis(rows); if(!out.summary || out.summary.unique_installs !== 1){process.exit(1);} const snap=generateInvestorSnapshots(rows,'weekly'); if(!snap.snapshots || snap.snapshots.length < 1){process.exit(1);} const cohorts=generateRetentionCohorts(rows); if(!cohorts.cohorts || cohorts.cohorts.length < 1){process.exit(1);} const ops=computeOpsSignals([{event:'query_answered',query_success:false,query_latency_ms:2200,timestamp:new Date().toISOString()},{event:'feedback_queue_health',pending_queue_size:40,oldest_pending_age_seconds:9000,timestamp:new Date().toISOString()}]); if(!ops.alerts || ops.alerts.length < 1){process.exit(1);} console.log('kpi-smoke-ok');"
