...
final class Database: @unchecked Sendable {
    ...
    private func runMigrations(from current: Int32, to target: Int32) throws {
        if current < 1 && target >= 1 {
            try applyMigrationStep(version: 1) {
                try execute("""
        ...
        if current < 2 && target >= 2 {
            try applyMigrationStep(version: 2) {
                if !table("knowledge_items", hasColumn: "content_hash") {
                    try execute("ALTER TABLE knowledge_items ADD COLUMN content_hash TEXT")
                }
                if !table("knowledge_items", hasColumn: "was_truncated") {
        ...
        if current < 4 && target >= 4 {
            try applyMigrationStep(version: 4) {
                try execute("""
        ...
        if current < 5 && target >= 5 {
            try applyMigrationStep(version: 5) {
    ...
    private func applyMigrationStep(version: Int32, _ block: () throws -> Void) throws {
        if try getSchemaVersion() >= version { return }
        try inTransaction {
            try block()
